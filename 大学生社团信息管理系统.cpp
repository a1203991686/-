...
char department1[6][10] = { "办公室", "外联", "宣传", "组织", "文艺", "财政" };
...
void choose_su_1(STA *head_s, STA_a *head_s_a); //超级管理员选单选项
void show_c_1(); //主席主选单
...
void show_m_1() //部长主选单
{
	printf("\t\t\t*************************************************\n");
	printf("\t\t\t\t\t\t部长主选单\n");
	printf("\t\t\t\t\t1. 加入本部学生信息\t2. 修改本部学生信息\n");
	printf("\t\t\t\t\t3. 删除本部学生信息\t4. 修改自己信息\n");
	printf("\t\t\t\t\t5. 修改自己密码    \t6. 修改部员密码\n");
	printf("\t\t\t\t\t0. 退出\n");
...
STA *create_s(STA *head_s, STA_a *head_s_a) //部员-创建
{
	...
	for (i = 0; i < n; i++)
	{
		...
		int department;
		scanf("%d", &department);
		while (department > 5 || department < 0)
		{
			printf("输入部门信息有误\n");
			printf("请重新输入第%d名同学部门\n", i + 1);
			scanf("%d", &department);
		}
		printf("\n");
		strcpy(p->num, num);
...
void change_s(STA *head_s, char num[]) //部员-修改
{
	...
	{
		printf("成员信息为");
		printf("%s\t%s\t%s\t%s\t%d-%d-%d\t%s\n", p->num, p->name, p->college, p->class1, p->day.year, p->day.month, p->day.day, department1[p->department]);
		printf("请输入姓名\n");
		char name[9]; //姓名
		scanf("%s", p->name);
		printf("请输入院系\n");
		...
		while (p->department > 5 || p->department < 0)
		{
			printf("输入部门信息有误\n");
			printf("请重新输入部门\n");
			scanf("%d", &p->department);
		...
		while (q)
		{
			printf("%s\t%s\t%s\t%s\t%d-%d-%d\t%s\n", q->num, q->name, q->college, q->class1, q->day.year, q->day.month, q->day.day, department1[q->department]);
			q = q->next;
		}
		getchar();
		save_s(head_s);
...
void save_s(STA *head_s) //部员-文件保存
{
	...
	while (sta)
	{
		...
		if (!p)
		{
			fprintf(fp, "%s\t%s\t%s\t%s\t%d-%d-%d\t%d", sta->num, sta->name, sta->college, \
...
STA *inf_s()  //部员-文件读取
{
	...
	{
		p = (STA *)malloc(sizeof(STA));
		fscanf(fp, "%s %s %s %s %d-%d-%d %d", p->num, p->name, p->college, \
...
void create_m(STA *head_s, STA_a *head_s_a) //部长-新建
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		min[i].department = p1->department;
		delete_s(head_s, head_s_a, num);
		create_m_a(num, i);
		printf("\n");
	}
...
void change_m(char num[]) //部长-修改
{
	...
	else
	{
		...
		scanf("%s", min[i].name);
		printf("请输入院系\n");
		scanf("%s", min[i].college);
		printf("请输入班级\n");
		scanf("%s", min[i].class1);
		printf("请输入出生年月日(yy mm dd)\n");
		scanf("%d %d %d", &min[i].day.year, &min[i].day.month, &min[i].day.day);
		save_m();
	}
}
...
void output_m() //部长-输出
{
	...
	for (int i = 0; i < 6; i++)
	{
		printf("%s\t%s\t%s\t%s\t%d-%d-%d\t%s\n", min[i].num, min[i].name, min[i].college, min[i].class1, min[i].day.year, min[i].day.month, min[i].day.day, department1[min[i].department]);
	}
	getchar();
}
...
void save_m() //部长-文件保存
{
	...
	for (i = 0; i < 6; i++)
	{
		if (i == 5)
		{
			fprintf(fp, "%s\t%s\t%s\t%s\t%d-%d-%d\t%d", min[i].num, min[i].name, min[i].college, \
...
void inf_m() //部长-文件读取
{
	...
	for (int i = 0; i < 6; i++)
	{
		fscanf(fp, "%s %s %s %s %d-%d-%d %d", min[i].num, min[i].name, min[i].college, \
			min[i].class1, &min[i].day.year, &min[i].day.month, &min[i].day.day, &min[i].department);
	}
	fclose(fp);
}
